...
//router.post('/',uploadFile.single("imageart"), authorizeRoles(["admin","user"],true),async (req, res) =>  {
router.post('/',uploadFile.single("imageart"),async (req, res) =>  {
    
    const {reference,designation,prix,marque,scategorieID,qtestock} = new Article(req.body)
    const imageart=req.file.filename
    const nouvarticle = new Article({reference:reference,designation:designation,prix:prix,marque:marque,imageart:imageart,scategorieID:scategorieID,qtestock:qtestock})
